...
import { EditableTitle } from "../../../../components/editable-title/EditableTitle.tsx";
...
export const Task: FC<TTaskProps> = memo(({ task }) => {
  ...
  const onChangeStatus = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(
      updateTaskTC(task.id, { ...task, isDone: event.target.checked }),
    ).catch();
  };
  ...
  const onUpdateTitle = () => {
    if (task.title !== title && title.trim().length > 4) {
      ...
      dispatch(updateTaskTC(task.id, { ...task, title })).catch(() =>
        setTitle(task.title),
      );
    } else {
      setTitle(task.title);
