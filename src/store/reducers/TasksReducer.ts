...
export const tasksReducer = (
  ...
  switch (type) {
    case "TASKS/SET-TASKS":
      return {
        ...state,
        tasks: payload.tasks.map((t) => ({ ...t, status: "idle" })),
      };
    case "TASKS/ADD-TASK":
    ...
    case "TASKS/SET-PRIORITY-FILTER":
      return {
        ...state,
        priorityFilter: state.priorityFilter === "desc" ? "asc" : "desc",
      };
    ...
    case "TASKS/SET-TASK-ERROR":
      return {
        ...state,
        tasks: state.tasks.map((t) =>
          t.id === payload.taskId ? { ...t, error: payload.error } : t,
        ),
      };
    default:
      return state;
...
export const updateTaskTC =
  (taskId: string, task: TTask) => (dispatch: Dispatch) => {
